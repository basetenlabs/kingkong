...
def is_lora_checkpoint(model_args):
    ...
    return (
        hasattr(model_args, "finetune_lora_rank") and model_args.finetune_lora_rank > 0
    )
...
def merge_lora_weights(state_dict, model_args):
    ...
    logger.info("=" * 60)
    logger.info(
        f"[LoRA] Scale: {scale:.4f} (alpha={model_args.finetune_lora_alpha}, rank={model_args.finetune_lora_rank})"
    )
    ...
    for lora_a_key in sorted(lora_a_keys):
        ...
        if not lora_module_name.startswith("finetune_lora_"):
            logger.warning(
                f"[LoRA] Skipping '{lora_module_name}' - doesn't start with 'finetune_lora_'"
            )
            continue

        base_relative = lora_module_name.replace("finetune_lora_", "").replace(
            "__", "."
        )
...
def convert_tt_to_hf_lora_path(prefix: str, base_relative: str) -> str:
    ...
    if "layers" in parts:
        ...
        if layer_idx_idx >= len(parts):
            logger.warning(
                f"[Path Mapping] Malformed prefix (missing layer idx): {prefix}"
            )
        ...
        if "experts" in parts:
            ...
            if base_relative not in proj_mapping:
                logger.warning(
                    f"[Path Mapping] Unknown expert projection '{base_relative}'"
                )
        ...
        if "mlp" in parts or "feed_forward" in parts:
            ...
            if base_relative not in proj_mapping:
                logger.warning(
                    f"[Path Mapping] Unknown MLP projection '{base_relative}'"
                )
...
def extract_lora_adapters(state_dict, model_args):
    ...
    logger.info(
        f"[LoRA] Rank: {model_args.finetune_lora_rank}, Alpha: {model_args.finetune_lora_alpha}"
    )
    logger.info(
        f"[LoRA] Scaling factor: {model_args.finetune_lora_alpha / model_args.finetune_lora_rank:.4f}"
    )
    ...
    logger.info(
        f"[LoRA] Found {len(lora_a_keys)} lora_A keys, {len(lora_b_keys)} lora_B keys"
    )
    ...
    all_lora_keys = [
        k for k in all_keys if "lora_" in k.lower() or "finetune_lora" in k
    ]
    if all_lora_keys:
...
def convert_to_hf_merged(
    ...
    logger.info(
        f"Starting DCP to HF conversion (merge mode): {input_dir} -> {output_dir}"
    )
...
def convert_to_hf_adapters(
    ...
    logger.info(
        f"LoRA config: rank={model_args.finetune_lora_rank}, alpha={model_args.finetune_lora_alpha}"
    )
    ...
    logger.info(
        f"Skipping {len(full_state_dict) - len(lora_keys)} base model keys (memory optimization)"
    )

    # Allocate real CPU tensors only for LoRA keys
    lora_state_dict = {
        k: torch.empty(
            full_state_dict[k].shape, dtype=full_state_dict[k].dtype, device="cpu"
        )
        for k in lora_keys
    }
    ...
    logger.info("Loaded LoRA weights successfully")

    # Extract and convert to PEFT format
    lora_state_dict, target_modules, _ = extract_lora_adapters(
        lora_state_dict, model_args
    )
    ...
    adapter_config = create_adapter_config(
        model_args, target_modules, base_model_name_or_path
    )
  ...
  python convert_to_hf.py checkpoint/ output/ \\
    ...
    try:
        ...
        if is_lora:
            # pyrefly: ignore [missing-attribute]
            logger.info(
                f"[Config] Detected LoRA checkpoint (rank={model_args.finetune_lora_rank})"
            )

            if args.merge_loras:
                # User explicitly wants to merge
                logger.info(
                    "[Config] Mode: --merge-loras (merging LoRA into base weights)"
                )
                ...
                if not args.adapters_only:
                    logger.info(
                        "[Config] Mode: adapters-only (default for LoRA checkpoints)"
                    )
                else:
            ...
            if args.adapters_only:
                parser.error(
                    "--adapters-only requires a LoRA checkpoint (finetune_lora_rank > 0)"
                )
            if args.merge_loras:
                logger.warning(
                    "[Config] --merge-loras specified but no LoRA weights in checkpoint"
                )
